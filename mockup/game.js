...
const ISLAND_START_Y = 145;
const STORAGE_CHEST = { x: 420, y: 211, width: 34, height: 25, hitPadding: 10 };
...
function shorelineY(x, t = 0) {
  return 101 + Math.sin((x + t * 6) * 0.04) * 6 + Math.sin((x - t * 3) * 0.013) * 4;
}

function isOnBeach(x, y) {
  return y >= shorelineY(x, state.time) && y <= ISLAND_START_Y;
}
...
function drawBeach(t) {
  ...
  ctx.lineTo(W, ISLAND_START_Y + 5);
  ctx.lineTo(0, ISLAND_START_Y + 5);
  ctx.closePath();
  ctx.fill();
  ...
  for (let y = 109; y < ISLAND_START_Y; y += 7) {
...
function drawIsland() {
  ...
  ctx.fillRect(0, ISLAND_START_Y, W, H - ISLAND_START_Y);
  for (let y = ISLAND_START_Y + 4; y < H; y += 8) {
  ...
  drawPalm(48, 190, 1);
  drawPalm(250, 178, .9);
  drawPalm(345, 235, 1.1);
  drawRock(24, 238, 14, 10);
  drawRock(167, 178, 10, 7);
  drawBush(112, 222);
  drawBush(459, 245);
  drawCampfire(235, 231);
  drawStorageChest();
}

function drawStorageChest() {
  const { x, y, width, height } = STORAGE_CHEST;
  px(x - width / 2, y - height / 2, width, height, "#51341c");
  px(x - width / 2 + 2, y - height / 2 + 2, width - 4, height - 5, "#8c5f2e");
  px(x - width / 2 + 2, y - 2, width - 4, 4, "#57371d");
  px(x - 3, y - 1, 6, 8, "#d2aa4e");
...
function drawPlacementMarker() {
  if (!state.selectedInventoryId || !state.pointer || state.inventoryOpen) return;
  const { x, y } = state.pointer;
  const clearOfItems = !state.beachItems.some((item) => Math.hypot(item.x - x, item.y - y) < 15);
  const canPlace = isOnBeach(x, y) && clearOfItems && state.beachItems.length < BEACH_ITEM_LIMIT;
  ctx.strokeStyle = canPlace ? "#b9f27c" : "#ef6b62";
  ctx.lineWidth = 2;
  ctx.beginPath();
...
function findOpenBeachPosition() {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    const y = Math.min(ISLAND_START_Y - 7, shore + 10 + Math.random() * Math.max(5, ISLAND_START_Y - 9 - shore));
...
function findClickedItem(x, y) {
  return [...state.beachItems].reverse().find((item) => Math.hypot(item.x - x, item.y - y) <= 14);
}

function isChestHit(x, y) {
  const { x: chestX, y: chestY, width, height, hitPadding } = STORAGE_CHEST;
  return Math.abs(x - chestX) <= width / 2 + hitPadding
    && Math.abs(y - chestY) <= height / 2 + hitPadding;
}

function setInventoryOpen(open) {
  state.inventoryOpen = open;
  document.getElementById("inventoryPanel").hidden = !open;
  document.getElementById("chestHint").style.opacity = open ? "0" : "1";
...
function updateStatus() {
  ...
  const message = now < state.messageUntil ? state.message : "漂着物をタップすると取得できます。";
  document.getElementById("driftStatus").textContent = `次の漂着：約${next}秒 ｜ 砂浜 ${state.beachItems.length}/${BEACH_ITEM_LIMIT}`;
...
function renderInventory() {
    ...
    if (item) {
      ...
      slot.addEventListener("click", () => {
        ...
        if (state.selectedInventoryId) {
          ...
          announce(`${item.definition.name}を選択中。砂浜をタップして配置します。`);
        }
      });
    } else {
      slot.textContent = "空き";
  ...
  document.getElementById("selection").textContent = selected
    ? `${selected.definition.name}を選択中。砂浜をタップして配置します。`
    : "アイテムを選ぶと砂浜に配置できます。";
...
canvas.addEventListener("pointermove", (event) => { state.pointer = canvasPoint(event); });
...
canvas.addEventListener("pointerdown", (event) => {
  ...
  announce(isOnBeach(x, y) ? "ここには漂着物がありません。" : "砂浜の漂着物をタップしてください。");
});

document.getElementById("toggleWave").addEventListener("click", () => {
  state.waveEnabled = !state.waveEnabled;
  document.getElementById("toggleWave").textContent = `波 ${state.waveEnabled ? "ON" : "OFF"}`;
});
...
async function initialize() {
  ...
  try {
    ...
    spawnRandomItem({ announceSpawn: false });
    spawnRandomItem({ announceSpawn: false });
    announce("漂着物をタップすると取得できます。木箱から持ち物を確認できます。", 5000);
    scheduleNextSpawn();
  } catch (error) {
